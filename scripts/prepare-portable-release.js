...
const { extractAndVerify } = require('../apps/dsa-desktop/portable-update/portableArchive');
...
async function main() {
  ...
  if (mode === 'stage') { fs.rmSync(target, { recursive: true, force: true }); fs.cpSync(source, target, { recursive: true, filter: (file) => !/(?:^|[\\/])(?:\.env|data|logs)(?:[\\/]|$)/i.test(path.relative(source, file)) });
