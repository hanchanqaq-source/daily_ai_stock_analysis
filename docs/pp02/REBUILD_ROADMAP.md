...
| R3｜按优先级迁移旧 PP02 功能 | 每个切片单独测试、CI 和 Judge | 进行中 | R3.1 已完成；R3.2 实现 Head 8/8 CI通过，等待最终Head收口 |
...
2. R3.2 手动默认与自动通知总开关：实现 Head 已通过，最终 Head CI 待收口。
3. R3.3 官方账本上的快捷持仓：下一切片。
4. R3.4 股票专用备份与恢复。
5. R3.5 应用内手动周期报告。
6. R3.6 Windows 便携更新。
7. R3.7 Windows 安全凭据。
