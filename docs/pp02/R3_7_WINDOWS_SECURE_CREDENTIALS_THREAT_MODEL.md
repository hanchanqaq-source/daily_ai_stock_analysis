...
| vault → backend child | 日志或命令行泄漏 | 只通过 child environment 注入；不放 argv；日志只记录数量/字段名，不记录值 |
...
2. Electron main 对敏感字段建立待提交事务，只在内存中生成密文。
3. 非敏感字段继续由现有 backend 配置服务写 `.env`。
4. Electron main 原子提交 vault，清除 `.env` 中同名敏感 assignment，并安全重启 backend。
5. 新 backend 从 vault 解密并通过 child environment 获得敏感值；设置页重新读取时只见 mask。
...
- 导入：非敏感项沿用版本冲突保护；Windows Desktop 遇到敏感项返回明确拒绝，不静默丢弃。
...
- CI contract：Windows Job 必须执行假密钥 harness，并且 artifact/log/source 扫描不包含假密钥明文。
...
4. 证明错误、stdout/stderr 和上传 artifact 不含假密钥明文。
