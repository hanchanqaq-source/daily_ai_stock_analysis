...
- [新功能] 周期报告写入正式数据库并支持按报告 ID 或最新记录直接重启读取，同一区间重复生成保持原报告 ID。
- [新功能] 新增版本化完整数据备份、预览和恢复入口，覆盖正式分析历史、组合事件、周期报告、支持的结构化记录及非敏感配置。
- [改进] 完整恢复在写入前保存当前状态保护副本，以数据库事务、配置内容哈希 CAS 和补偿机制避免损坏包、过期预览或中断破坏现有数据。
- [改进] 设置页明确区分完整数据备份、非敏感配置备份和股票组合账本备份，并明确排除凭据、运行路径、缓存、日志和草稿。
