Warning: truncated output (original token count: 52514)
Total output lines: 2220
...
- ⚙️ **GitHub Actions 分析任务超时支持 `vars` 配…2514 tokens truncated…DIATELY`、`RUN_IMMEDIATELY` 后，`Config` 会优先读取持久化 `.env` 中的新值，避免被容器创建时注入的旧环境变量覆盖。
