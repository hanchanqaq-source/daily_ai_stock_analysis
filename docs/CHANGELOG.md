...
- [修复] Windows 安装、正常卸载和失败清理子进程改为有界等待与精确进程树终止；生命周期步骤增加外层看门狗并继续上传脱敏阶段诊断，避免挂起后无证据。
- [修复] Desktop 安全凭据提交按后端规范内容代数校验配置保存结果，并继续使用兼容代数绑定 DPAPI vault，修复有效 AI 配置保存后被误判版本冲突。
- [测试] Windows 候选新增安装态 AIHubMix 校验/保存、真实 safeStorage、重启遮罩、回环 LiteLLM 鉴权请求、配置与完整备份导出、假凭据泄漏扫描及一次正常卸载的固定 Head 验收链。
...
- [新功能] 新增版本化完整数据备份与恢复，使用单一 SQLite 读快照、规范 JSON、清单和 SHA-256 覆盖正式分析、股票组合事件、周期报告、会话消息与摘要、结构化用户记录及非敏感配置，并明确排除凭据、Agent Provider 跟踪、草稿和运行时路径。
- [改进] 周期报告改为规范持久化并支持重启后只读加载；v3.29.2 迁移保持既有分析历史不变，旧展望快照仅作兼容读取。
- [测试] 补充固定 v3.29.2 人工数据库经正式 API 完整导出到安装目录外、清洁安装恢复、正式读取的端到端验收，以及完整备份校验、恢复前证据、异常中断回滚、并发写入/一致性快照、重启持久化、迁移和 Web 预览确认测试，并隔离测试进程中的人工假配置环境。
- [文档] 说明完整备份、仅配置备份和仅组合事件备份的区别，以及外部存储、恢复预览、重启、恢复文件、兼容性和基金不适用边界。
- [修复] 完整恢复新增启动前持久事务日志与 SQLite 提交标记，进程被强制终止后按三方比较安全回滚或完成非敏感配置，同时保留并发配置写入；配置导出与写入规范化保持一致，恢复会清理被明确排除的 Agent Provider 跟踪。
- [修复] Windows 官方卸载器通过随包固定清单和自定位助手关闭精确归属的 Electron/后端进程，并保留脱敏的初始/归零计数证据；严格验收只执行一次卸载，不依赖重试、NSIS 身份参数或按进程名广泛清理。
- [修复] Windows 候选的 EXE 文件版本、产品版本、界面版本、构建信息和清单统一为 `3.29.3`，并加入产物级一致性校验。
- [修复] 完整备份纳入不可安全重建的 `fundamental_snapshot`；`stock_daily` 仅作为无用户数据的可重建行情缓存排除，并在清单声明、恢复清理及按需重建测试中闭环。
- [测试] Windows 候选验证新增只读 Authenticode 状态与可选有效签名策略门，不读取证书、私钥或 CI Secret；真实签名身份仍需单独授权。
- [修复] Windows 打包版在启动后端前校验桌面端与冻结后端的固定路径、文件大小和 SHA-256；桌面参数不完整或文件身份不一致时在业务初始化前停止，并显示明确中文提示。
- [测试] Windows CI 与正式发布流程在最终免安装 ZIP 解压后复用运行时身份校验，再执行冻结后端探针，阻止文件身份漂移的候选上传或发布。
- [修复] Windows 冻结后端启动改用单调时钟轮询真实 Uvicorn 就绪状态，并将冲突的内部 3 秒上限改为有界 30 秒，避免慢速 CI 或工作电脑在外层健康门仍允许等待时被提前终止。
- [修复] Windows Desktop 首次保存 AI 渠道时以不落盘的 mask 占位完成后端交叉校验，再提交 DPAPI 安全凭据；配置失败会显示具体字段和原因，不再只显示笼统错误。
