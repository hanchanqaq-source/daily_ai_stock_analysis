# PP02 滚动路线图

已完成并验收的历史不得倒改；未完成项可在用户按三选一规则作出决策后重排或拆分。

## Work 1｜官方稳定版干净底座建立

| 子步骤 | 内容 | 状态 | 完成条件 |
| --- | --- | --- | --- |
| Work 1.1 | 恢复上下文、确认角色、检查目录与新旧项目边界 | 完成 | 原 WORK-001 启动会话确认为 `CHAT_ROLE=WORK`、`WORK_ID=WORK-001`，旧仓库未修改；不替代新会话独立判定 |
| Work 1.2 | 核对 Release、Tag、Commit、License、安全信息及 `main` 差异 | 完成 | 锁定 `v3.27.0 / b36c7214…`，新增提交只作候选 |
| Work 1.3 | 新仓库授权门 | 完成 | 已授权私有仓库方案 A 与首次导入/推送范围 |
| Work 1.4.1 | 从官方稳定 Tag 建立原始底座并保留历史 | 部分完成 | 新仓库 `main` 已准确指向并可追溯到官方 Tag Commit；目标仓库仍需建立 `v3.27.0` Tag |
| Work 1.4.2 | 增加项目级 `AGENTS.md` 与台账导航 | 本地完成 | 管理文件完整且不含业务功能修改；待发布到已校正的远端底座 |
| Work 1.4.3 | 与官方 Tag 比对 | 本地完成 | 除批准的 9 个管理/文档文件外，业务代码零差异；待管理层发布后远端复核 |
| Work 1.4.4 | 新测试会话验证规则识别 | 完成 | 独立只读会话正确识别项目、默认角色、边界、Work、三选一、授权门与唯一台账 |
| Work 1.5 | 安装、测试、后端、Web、目录与跨项目引用验证 | 云端完成 | 云端验证全部通过；Windows 项进入实机验收 |
| Work 1.6 | 台账、CI、Judge 与总控交付总结 | 未开始 | CI/Judge/阻塞/未验证项全部有记录 |

## Work 2 及以后

Work 1 完成前不启动 Work 2。后续详细步骤由项目总控依据已确认需求派发，不在本文件提前虚构。

已登记但未排期的上游候选补丁：

- Windows `mimetypes` 启动卡死修复
- 美股、港股英文新闻相关性修复
