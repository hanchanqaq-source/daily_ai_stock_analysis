...
Work32 is the active Draft-only P002 final usability closure from exact
`main@295821e4…`. It fixes the unbounded Windows install/uninstall waits that left
main Run `31211548666` non-terminal, and the mismatched backend/Desktop config
generation algorithms that rejected secure credential commit after a valid
save. Local Desktop `136/136`, Web `1081 passed / 2 skipped`, lint/build and 28
Python/Actions contracts pass. Exact-Head remote CI, Windows DPAPI installed
save/restart/smoke/export/uninstall evidence, Defender reports, candidate hashes
and leakage scans remain mandatory. Judge stays `DRAFT_HOLD`; there is no Ready,
merge, Tag, or Release authorization.
...
已以测试先行收口 Windows 冻结筹码运行时；固定 Head `016e3408…` 的 Run
`30831393819` 为 7 success、1 path-skipped，直接冻结与最终 ZIP 探针通过，依赖不变，
裁决 `PASS — DRAFT_HOLD`。
...
| Work32｜P002 final usability closure | Local hard gates pass / Draft Hold | One exact-Head Draft PR, native CI, Defender and safe candidate evidence pending; no usability claim, merge or release |
