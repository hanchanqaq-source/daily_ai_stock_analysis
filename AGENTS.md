...
### 0.1 项目身份与 Work 自动接力（PP02-WORK-HANDOFF-002 v1.0）
...
- “总控”是每次施工前自动执行的范围、状态、风险与授权检查，不再绑定某个聊天。
  `Work1`、`Work2`、`Work3` 等只表示路线中的完整大段，也不赋予聊天永久权限。
- 旧的“总控聊天不得施工”“Work 聊天不可复用”“必须人工复制施工单和回传报告”
  以及 `CHAT_ROLE` / `ROLE_LOCK` 窗口锁规则保留为历史，状态为
  `Superseded by PP02-WORK-HANDOFF-002`，不得继续作为活动规则。
- 同一项目中任意具备仓库执行能力的 Codex 聊天都可接管，但同一时间只能有一个
  Active Work。聊天显示名称属于用户界面数据，任何自动流程都不得修改、建议修改
  或把逻辑 Work 名称冒充为聊天重命名结果。
- 一个完整大段使用一个 Work 聊天。当前 Work 必须连续完成接管、Plan、Build、
  Test、CI、Judge 和交接；完整结束后释放施工权，并明确要求用户新开同项目聊天，
  只发送“下一步”。上下文过长但大段未验收时只能标记“续接”，不得冒充结束。

### 0.2 当前状态与交接

- `_ai-dev/PROJECT_STATUS.md` 是唯一当前状态真源。其他台账只承担需求、历史、
  阻塞或导航职责；发生冲突时，先依据可验证证据修正状态，不得直接宣称完成。
- 新聊天必须依次读取 `AGENTS.md`、`_ai-dev/PROJECT_STATUS.md`、
  `_ai-dev/AI_HANDOFF.md`、`_ai-dev/WORK_TASK.md`、`_ai-dev/WORK_RETURN.md`、
  正式路线图，再核对 GitHub 的真实 `main`、Open PR、固定 Head 和 CI。
- 接管顺序以 GitHub 事实、唯一状态、已批准路线、当前聊天、其他聊天检索结果为
  优先级；聊天摘要只作补充，不能覆盖仓库与 GitHub 证据。
- `_ai-dev/PROJECT_STATUS.md` 至少记录当前 Work、状态、Base、Branch、PR、Active
  Goal、下一动作、是否等待授权和最后更新时间。发现另一个 Active Work 时，当前
  聊天只能只读报告，不得重复施工；疑似卡死先核对 GitHub，再决定是否申请接管。
- 每个 Work 在 `_ai-dev/WORK_TASK.md` 固定目标、范围、非目标、验收和授权，
  在 `_ai-dev/WORK_RETURN.md` 记录真实结果、未验证项、阻塞与 Judge。
- 施工单、决定和完成报告必须自动写入上述现有真源；不得要求用户在聊天间复制
  长指令、PR/CI 信息或完成报告。
...
- 用户说“下一步”或“继续”：自动接管并在当前授权范围内连续执行，直到下一个
  重要决策门、硬授权门、真实阻塞或本 Work 完整结束。
...
### 0.6 统一执行端自动路由（PP02-AUTO-ROUTER-001 v1.2）

- `AUTO_ROUTING=TRUE`、`USER_SELECTS_TOOL=FALSE`、`INHERITANCE=ALL_WORKS`。
  用户只需提出目标或发送“下一步”“继续”“暂停”“停止”，不负责选择执行工具。
- 当前 Active Work 自动执行总控检查、掌握完整路线、划分大步、决定执行端、审核
  结果和管理授权门；Codex 负责代码、终端、调试、构建、
...
- 后续 Work 继承本路由与自动接力机制，但不自动继承业务范围、仓库/分支/PR、
  真实数据、本机目录、大型依赖、Ready/合并/Release、付费服务或不可逆授权；
  每个 Work 接管时从状态真源和 GitHub 重新确认目标、环境和有效授权。
...
- 本规则只调整未完成路线和未来 Work；R0 及其他已验收历史不得回退或重做。
  v1.1 的工具自动路由继续有效；其中绑定总控/Work 聊天的表述由 v1.2 和
  `PP02-WORK-HANDOFF-002` 替代。
