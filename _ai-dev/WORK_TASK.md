...
实现 Windows 便携 ZIP 安全更新；当前只在 Draft PR #7 内归并已批准的五项治理
硬门并修正 PR #6 遗留状态，完成后等待 R5 Windows 实机验收授权。

本次归并只强化现有 PP02 控制层，不改变官方 `v3.28.0` 业务底座，不修改已经
通过 CI 的 R3.6 便携更新业务逻辑，也不进入 R3.7。
...
- 在现有 `AGENTS.md` Overlay、四份 `_ai-dev` 文件、既有路线图、项目入口和
  追加式变更历史中应用治理硬门。
- 在当前 Draft PR `#7` / `codex-4z7ady` 上执行 R3.6 范围内的测试、治理文档、
  普通 Commit、Push、CI 检查和范围内修复。
